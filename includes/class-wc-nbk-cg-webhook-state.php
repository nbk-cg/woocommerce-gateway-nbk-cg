 ...
 */
class WC_Nbk_Cg_Webhook_State {
	const OPTION_LIVE_MONITORING_BEGAN_AT = 'wc_Nbk_Cg_wh_monitor_began_at';
	const OPTION_LIVE_LAST_SUCCESS_AT     = 'wc_Nbk_Cg_wh_last_success_at';
	const OPTION_LIVE_LAST_FAILURE_AT     = 'wc_Nbk_Cg_wh_last_failure_at';
	const OPTION_LIVE_LAST_ERROR          = 'wc_Nbk_Cg_wh_last_error';

	const OPTION_TEST_MONITORING_BEGAN_AT = 'wc_Nbk_Cg_wh_test_monitor_began_at';
	const OPTION_TEST_LAST_SUCCESS_AT     = 'wc_Nbk_Cg_wh_test_last_success_at';
	const OPTION_TEST_LAST_FAILURE_AT     = 'wc_Nbk_Cg_wh_test_last_failure_at';
	const OPTION_TEST_LAST_ERROR          = 'wc_Nbk_Cg_wh_test_last_error';
	...
	public static function get_last_error_reason() {
		...
		if ( self::VALIDATION_SUCCEEDED == $last_error ) {
			return( __( 'No error', 'woocommerce-gateway-nbk-cg' ) );
		}

		if ( self::VALIDATION_FAILED_EMPTY_HEADERS == $last_error ) {
			return( __( 'The webhook was missing expected headers', 'woocommerce-gateway-nbk-cg' ) );
		}

		if ( self::VALIDATION_FAILED_EMPTY_BODY == $last_error ) {
			return( __( 'The webhook was missing expected body', 'woocommerce-gateway-nbk-cg' ) );
		}
		...
		if ( self::VALIDATION_FAILED_SIGNATURE_INVALID == $last_error ) {
			return( __( 'The webhook signature was missing or was incorrectly formatted', 'woocommerce-gateway-nbk-cg' ) );
		}

		if ( self::VALIDATION_FAILED_TIMESTAMP_MISMATCH == $last_error ) {
			return( __( 'The timestamp in the webhook differed more than five minutes from the site time', 'woocommerce-gateway-nbk-cg' ) );
		}

		if ( self::VALIDATION_FAILED_SIGNATURE_MISMATCH == $last_error ) {
			return( __( 'The webhook was not signed with the expected signing secret', 'woocommerce-gateway-nbk-cg' ) );
		}

		return( __( 'Unknown error.', 'woocommerce-gateway-nbk-cg' ) );
	}
	...
	public static function get_webhook_status_message() {
		...
		if ( $last_success_at > $last_failure_at ) {
			$message = sprintf(
				$test_mode ?
					...
					__( 'The most recent test webhook, timestamped %s, was processed successfully.', 'woocommerce-gateway-nbk-cg' ) :
					/* translators: 1) date and time of last webhook received, e.g. 2020-06-28 10:30:50 UTC */
					__( 'The most recent live webhook, timestamped %s, was processed successfully.', 'woocommerce-gateway-nbk-cg' ),
		...
		if ( ( 0 == $last_success_at ) && ( 0 == $last_failure_at ) ) {
			$message = sprintf(
				$test_mode ?
					...
					__( 'No test webhooks have been received since monitoring began at %s.', 'woocommerce-gateway-nbk-cg' ) :
					/* translators: 1) date and time webhook monitoring began, e.g. 2020-06-28 10:30:50 UTC */
					__( 'No live webhooks have been received since monitoring began at %s.', 'woocommerce-gateway-nbk-cg' ),
		...
		if ( $last_success_at > 0 ) {
			$message = sprintf(
				$test_mode ?
					 ...
					 */
					__( 'Warning: The most recent test webhook, received at %1$s, could not be processed. Reason: %2$s. (The last test webhook to process successfully was timestamped %3$s.)', 'woocommerce-gateway-nbk-cg' ) :
					/*
					 * translators: 1) date and time of last failed webhook e.g. 2020-06-28 10:30:50 UTC
					 * translators: 2) reason webhook failed
					 * translators: 3) date and time of last successful webhook e.g. 2020-05-28 10:30:50 UTC
					 */
					__( 'Warning: The most recent live webhook, received at %1$s, could not be processed. Reason: %2$s. (The last live webhook to process successfully was timestamped %3$s.)', 'woocommerce-gateway-nbk-cg' ),
		...
		$message = sprintf(
			$test_mode ?
				 ...
				 */
				__( 'Warning: The most recent test webhook, received at %1$s, could not be processed. Reason: %2$s. (No test webhooks have been processed successfully since monitoring began at %3$s.)', 'woocommerce-gateway-nbk-cg' ) :
				/* translators: 1) date and time of last failed webhook e.g. 2020-06-28 10:30:50 UTC
				 * translators: 2) reason webhook failed
				 * translators: 3) date and time webhook monitoring began e.g. 2020-05-28 10:30:50 UTC
				 */
				__( 'Warning: The most recent live webhook, received at %1$s, could not be processed. Reason: %2$s. (No live webhooks have been processed successfully since monitoring began at %3$s.)', 'woocommerce-gateway-nbk-cg' ),
			gmdate( $date_format, $last_failure_at ),
			$last_error,
			gmdate( $date_format, $monitoring_began_at )
		);
