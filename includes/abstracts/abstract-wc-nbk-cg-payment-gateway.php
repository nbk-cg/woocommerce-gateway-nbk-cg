 ...
 * @since 4.0.0
 */
abstract class WC_Nbk_Cg_Payment_Gateway extends WC_Payment_Gateway_CC {
	/**
	...
	public function save_payment_method_checkbox() {
		printf(
			...
			esc_attr( $this->id ),
			esc_html( apply_filters( 'wc_nbk_cg_save_to_account_text', __( 'Save payment information to my account for future purchases.', 'woocommerce-gateway-nbk-cg' ) ) )
		);
	}
	...
	public function are_keys_set() {
		...
		// in includes/payment-methods/class-wc-nbk-cg-payment-request.php
	...
	public function maybe_process_pre_orders( $order_id ) {
		return (
			WC_Nbk_Cg_Helper::is_pre_orders_exists() &&
	...
	public function payment_icons() {
		return apply_filters(
			'wc_Nbk_Cg_payment_icons',
			[
				'visa'       => '<img src="' . WC_NBK_CG_PLUGIN_URL . '/assets/images/visa.svg" class="nbk-cg-visa-icon nbk-cg-icon" alt="Visa" />',
				'amex'       => '<img src="' . WC_NBK_CG_PLUGIN_URL . '/assets/images/amex.svg" class="nbk-cg-amex-icon nbk-cg-icon" alt="American Express" />',
				'mastercard' => '<img src="' . WC_NBK_CG_PLUGIN_URL . '/assets/images/mastercard.svg" class="nbk-cg-mastercard-icon nbk-cg-icon" alt="Mastercard" />',
				'discover'   => '<img src="' . WC_NBK_CG_PLUGIN_URL . '/assets/images/discover.svg" class="nbk-cg-discover-icon nbk-cg-icon" alt="Discover" />',
				'alipay'     => '<img src="' . WC_NBK_CG_PLUGIN_URL . '/assets/images/alipay.svg" class="nbk-cg-alipay-icon nbk-cg-icon" alt="Alipay" />',
                'mtn'        => '<img src="' . WC_NBK_CG_PLUGIN_URL . '/assets/images/mtn.svg" class="nbk-cg-mtn-icon nbk-cg-icon" alt="Mtn" />',
                'paypal'     => '<img src="' . WC_NBK_CG_PLUGIN_URL . '/assets/images/paypal.svg" class="nbk-cg-paypal-icon nbk-cg-icon" alt="PayPal" />',
			]
		);
	}
	...
	public function validate_minimum_order_amount( $order ) {
		if ( $order->get_total() * 100 < WC_Nbk_Cg_Helper::get_minimum_amount() ) {
			/* translators: 1) amount (including currency symbol) */
			throw new WC_Nbk_Cg_Exception( 'Did not meet minimum amount', sprintf( __( 'Sorry, the minimum allowed order total is %1$s to use this payment method.', 'woocommerce-gateway-nbk-cg' ), wc_price( WC_Nbk_Cg_Helper::get_minimum_amount() / 100 ) ) );
		}
	}
	...
	public function get_transaction_url( $order ) {
		if ( $this->testmode ) {
			$this->view_transaction_url = 'https://dashboard.nbk-cg.com/test/payments/%s';
		} else {
			$this->view_transaction_url = 'https://dashboard.nbk-cg.com/payments/%s';
		}
	 ...
	 * @version 4.0.0
	 */
	public function get_nbk_cg_customer_id( $order ) {
		$customer = get_user_option( '_nbk_cg_customer_id', $order->get_customer_id() );

		if ( empty( $customer ) ) {
			// Try to get it via the order.
			return $order->get_meta( '_nbk_cg_customer_id', true );
		} else {
			return $customer;
		}
	 ...
	 */
	public function get_Nbk_Cg_return_url( $order = null, $id = null ) {
		if ( is_object( $order ) ) {
			if ( empty( $id ) ) {
				$id = uniqid();
			}
	...
	public function generate_payment_request( $order, $prepared_source ) {
		$settings              = get_option( 'woocommerce_nbk_cg_settings', [] );
		$statement_descriptor  = ! empty( $settings['statement_descriptor'] ) ? str_replace( "'", '', $settings['statement_descriptor'] ) : '';
		$capture               = ! empty( $settings['capture'] ) && 'yes' === $settings['capture'] ? true : false;
		$post_data             = [];
		$post_data['currency'] = strtolower( $order->get_currency() );
		$post_data['amount']   = WC_Nbk_Cg_Helper::get_nbk_cg_amount( $order->get_total(), $post_data['currency'] );
		/* translators: 1) blog name 2) order number */
		$post_data['description'] = sprintf( __( '%1$s - Order %2$s', 'woocommerce-gateway-nbk-cg' ), wp_specialchars_decode( get_bloginfo( 'name' ), ENT_QUOTES ), $order->get_order_number() );
		$billing_email            = $order->get_billing_email();
		$billing_first_name       = $order->get_billing_first_name();
		$billing_last_name        = $order->get_billing_last_name();

		if ( ! empty( $billing_email ) && apply_filters( 'wc_Nbk_Cg_send_Nbk_Cg_receipt', false ) ) {
		...
		switch ( $order->get_payment_method() ) {
			case 'stripe':
				if ( ! empty( $statement_descriptor ) ) {
					$post_data['statement_descriptor'] = WC_Nbk_Cg_Helper::clean_statement_descriptor( $statement_descriptor );
				}
		...
		$metadata = [
			__( 'customer_name', 'woocommerce-gateway-nbk-cg' ) => sanitize_text_field( $billing_first_name ) . ' ' . sanitize_text_field( $billing_last_name ),
			__( 'customer_email', 'woocommerce-gateway-nbk-cg' ) => sanitize_email( $billing_email ),
		...
		$post_data['metadata'] = apply_filters( 'wc_Nbk_Cg_payment_metadata', $metadata, $order, $prepared_source );
		 ...
		 * @param object $source
		 */
		return apply_filters( 'wc_Nbk_Cg_generate_payment_request', $post_data, $order, $prepared_source );
	}
	...
	public function process_response( $response, $order ) {
		WC_Nbk_Cg_Logger::log( 'Processing response: ' . print_r( $response, true ) );

		$order_id = $order->get_id();
		$captured = ( isset( $response->captured ) && $response->captured ) ? 'yes' : 'no';

		// Store charge data.
		$order->update_meta_data( '_Nbk_Cg_charge_captured', $captured );
		...
		if ( 'yes' === $captured ) {
			...
			if ( 'pending' === $response->status ) {
				...
				$order->update_status( 'on-hold', sprintf( __( 'Stripe charge awaiting payment: %s.', 'woocommerce-gateway-nbk-cg' ), $response->id ) );
			}
			...
			if ( 'succeeded' === $response->status ) {
				...
				/* translators: transaction id */
				$message = sprintf( __( 'Stripe charge complete (Charge ID: %s)', 'woocommerce-gateway-nbk-cg' ), $response->id );
				$order->add_order_note( $message );
			}

			if ( 'failed' === $response->status ) {
				$localized_message = __( 'Payment processing failed. Please retry.', 'woocommerce-gateway-nbk-cg' );
				$order->add_order_note( $localized_message );
				throw new WC_Nbk_Cg_Exception( print_r( $response, true ), $localized_message );
			}
		} else {
			$order->set_transaction_id( $response->id );
			...
			/* translators: transaction id */
			$order->update_status( 'on-hold', sprintf( __( 'Stripe charge authorized (Charge ID: %s). Process order to take payment, or cancel to remove the pre-authorization. Attempting to refund the order in part or in full will release the authorization and cancel the payment.', 'woocommerce-gateway-nbk-cg' ), $response->id ) );
		}
		...
		do_action( 'WC_Gateway_Nbk_Cgprocess_response', $response, $order );
	...
	public function get_owner_details( $order ) {
		...
		return (object) apply_filters( 'wc_Nbk_Cg_owner_details', $details, $order );
	}
	...
	public function get_source_object( $source_id = '' ) {
		...
		$source_object = WC_Nbk_Cg_API::retrieve( 'sources/' . $source_id );

		if ( ! empty( $source_object->error ) ) {
			throw new WC_Nbk_Cg_Exception( print_r( $source_object, true ), $source_object->error->message );
		}
	...
	public function prepare_source( $user_id, $force_save_source = false, $existing_customer_id = null ) {
		...
		$force_save_source = apply_filters( 'wc_Nbk_Cg_force_save_source', $force_save_source, $customer );
		$source_object     = '';
		$source_id         = '';
		$wc_token_id       = false;
		$payment_method    = isset( $_POST['payment_method'] ) ? wc_clean( wp_unslash( $_POST['payment_method'] ) ) : 'stripe';
		...
		if ( ! empty( $_POST['stripe_source'] ) ) {
			...
			if ( ! $wc_token || $wc_token->get_user_id() !== get_current_user_id() ) {
				...
				throw new WC_Nbk_Cg_Exception( 'Invalid payment method', __( 'Invalid payment method. Please input a new card number.', 'woocommerce-gateway-nbk-cg' ) );
			}
	...
	public function prepare_order_source( $order = null ) {
		$stripe_customer = new WC_Nbk_Cg_Customer();
		$stripe_source   = false;
		$token_id        = false;
		$source_object   = false;
		...
		if ( $order ) {
			...
			$stripe_customer_id = get_post_meta( $order_id, '_nbk_cg_customer_id', true );

			if ( $stripe_customer_id ) {
				$stripe_customer->set_id( $stripe_customer_id );
			}

			$source_id = $order->get_meta( '_nbk_cg_source_id', true );

			// Since 4.0.0, we changed card to source so we need to account for that.
			if ( empty( $source_id ) ) {
				$source_id = $order->get_meta( '_Nbk_Cg_card_id', true );

				// Take this opportunity to update the key name.
				$order->update_meta_data( '_nbk_cg_source_id', $source_id );
			...
			if ( $source_id ) {
				...
				$source_object = WC_Nbk_Cg_API::retrieve( 'sources/' . $source_id );
			} elseif ( apply_filters( 'wc_Nbk_Cg_use_default_customer_source', true ) ) {
				/*
	...
	public function save_source_to_order( $order, $source ) {
		...
		if ( $source->customer ) {
			$order->update_meta_data( '_nbk_cg_customer_id', $source->customer );
		}

		if ( $source->source ) {
			$order->update_meta_data( '_nbk_cg_source_id', $source->source );
		}
	...
	public function update_fees( $order, $balance_transaction_id ) {
		$balance_transaction = WC_Nbk_Cg_API::retrieve( 'balance/history/' . $balance_transaction_id );

		if ( empty( $balance_transaction->error ) ) {
			if ( isset( $balance_transaction ) && isset( $balance_transaction->fee ) ) {
				// Fees and Net needs to both come from Stripe to be accurate as the returned
				// values are in the local currency of the Stripe account, not from WC.
				$fee_refund = ! empty( $balance_transaction->fee ) ? WC_Nbk_Cg_Helper::format_balance_fee( $balance_transaction, 'fee' ) : 0;
				$net_refund = ! empty( $balance_transaction->net ) ? WC_Nbk_Cg_Helper::format_balance_fee( $balance_transaction, 'net' ) : 0;

				// Current data fee & net.
				$fee_current = WC_Nbk_Cg_Helper::get_Nbk_Cg_fee( $order );
				$net_current = WC_Nbk_Cg_Helper::get_Nbk_Cg_net( $order );

				// Calculation.
				$fee = (float) $fee_current + (float) $fee_refund;
				$net = (float) $net_current + (float) $net_refund;

				WC_Nbk_Cg_Helper::update_Nbk_Cg_fee( $order, $fee );
				WC_Nbk_Cg_Helper::update_Nbk_Cg_net( $order, $net );

				$currency = ! empty( $balance_transaction->currency ) ? strtoupper( $balance_transaction->currency ) : null;
				WC_Nbk_Cg_Helper::update_Nbk_Cg_currency( $order, $currency );
			...
			WC_Nbk_Cg_Logger::log( 'Unable to update fees/net meta for order: ' . $order->get_id() );
		}
	}
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		$captured       = $order->get_meta( '_Nbk_Cg_charge_captured', true );
		...
		if ( ! is_null( $amount ) ) {
			$request['amount'] = WC_Nbk_Cg_Helper::get_nbk_cg_amount( $amount, $order_currency );
		}
		...
		WC_Nbk_Cg_Logger::log( "Info: Beginning refund for order {$charge_id} for the amount of {$amount}" );

		$request = apply_filters( 'wc_Nbk_Cg_refund_request', $request, $order );
		...
		if ( ! $intent_cancelled && 'yes' === $captured ) {
			$response = WC_Nbk_Cg_API::request( $request, 'refunds' );
		}

		if ( ! empty( $response->error ) ) {
			WC_Nbk_Cg_Logger::log( 'Error: ' . $response->error->message );

			return new WP_Error(
				'stripe_error',
				sprintf(
					/* translators: %1$s is a stripe error message */
					__( 'There was a problem initiating a refund: %1$s', 'woocommerce-gateway-nbk-cg' ),
			...
			if ( in_array( strtolower( $order->get_currency() ), WC_Nbk_Cg_Helper::no_decimal_currencies() ) ) {
			...
			if ( 'yes' !== $captured ) {
				...
				$order->add_order_note( sprintf( __( 'Pre-Authorization for %s voided.', 'woocommerce-gateway-nbk-cg' ), $formatted_amount ) );
				...
				if ( ! is_null( $amount ) ) {
					...
					throw new Exception( __( 'The authorization was voided and the order cancelled. Click okay to continue, then refresh the page.', 'woocommerce-gateway-nbk-cg' ) );
				} else {
			...
			$order->update_meta_data( '_Nbk_Cg_refund_id', $response->id );
			...
			$refund_message = sprintf( __( 'Refunded %1$s - Refund ID: %2$s - Reason: %3$s', 'woocommerce-gateway-nbk-cg' ), $formatted_amount, $response->id, $reason );

			$order->add_order_note( $refund_message );
			WC_Nbk_Cg_Logger::log( 'Success: ' . html_entity_decode( wp_strip_all_tags( $refund_message ) ) );
	...
	public function add_payment_method() {
		$error     = false;
		$error_msg = __( 'There was a problem adding the payment method.', 'woocommerce-gateway-nbk-cg' );
		$source_id = '';
		...
		$source_object = WC_Nbk_Cg_API::retrieve( 'sources/' . $source );
		...
		if ( $error ) {
			...
			WC_Nbk_Cg_Logger::log( 'Add payment method Error: ' . $error_msg );
			return;
		}

		do_action( 'wc_Nbk_Cg_add_payment_method_' . ( isset( $_POST['payment_method'] ) ? wc_clean( wp_unslash( $_POST['payment_method'] ) ) : '' ) . '_success', $source_id, $source_object );
	...
	public function generate_create_intent_request( $order, $prepared_source ) {
		...
		$request = [
			...
			'amount'               => WC_Nbk_Cg_Helper::get_nbk_cg_amount( $order->get_total() ),
		...
		$force_save_source = apply_filters( 'wc_Nbk_Cg_force_save_source', false, $prepared_source->source );
		 ...
		 * @param object $source
		 */
		return apply_filters( 'wc_Nbk_Cg_generate_create_intent_request', $request, $order, $prepared_source );
	}
	...
	public function get_level3_data_from_order( $order ) {
		...
		$stripe_line_items = array_map(
			function( $item ) use ( $currency ) {
				...
				$unit_cost           = WC_Nbk_Cg_Helper::get_nbk_cg_amount( ( $subtotal / $quantity ), $currency );
				$tax_amount          = WC_Nbk_Cg_Helper::get_nbk_cg_amount( $item->get_total_tax(), $currency );
				$discount_amount     = WC_Nbk_Cg_Helper::get_nbk_cg_amount( $subtotal - $item->get_total(), $currency );
		...
		$level3_data = [
			...
			'shipping_amount'    => WC_Nbk_Cg_Helper::get_nbk_cg_amount( (float) $order->get_shipping_total() + (float) $order->get_shipping_tax(), $currency ), // The shipping cost, in cents, as a non-negative integer.
	...
	public function create_intent( $order, $prepared_source ) {
		...
		$intent = WC_Nbk_Cg_API::request( $request, 'payment_intents' );
		if ( ! empty( $intent->error ) ) {
			return $intent;
		}

		$order_id = $order->get_id();
		WC_Nbk_Cg_Logger::log( "Stripe PaymentIntent $intent->id initiated for order $order_id" );
	...
	public function update_existing_intent( $intent, $order, $prepared_source ) {
		...
		$new_amount = WC_Nbk_Cg_Helper::get_nbk_cg_amount( $order->get_total() );
		...
		return WC_Nbk_Cg_API::request_with_level3_data(
			$request,
	...
	public function confirm_intent( $intent, $order, $prepared_source ) {
		...
		$level3_data      = $this->get_level3_data_from_order( $order );
		$confirmed_intent = WC_Nbk_Cg_API::request_with_level3_data(
			$confirm_request,
		...
		if ( 'succeeded' === $confirmed_intent->status ) {
			WC_Nbk_Cg_Logger::log( "Stripe PaymentIntent $intent->id succeeded for order $order_id" );
		} elseif ( 'requires_action' === $confirmed_intent->status ) {
			WC_Nbk_Cg_Logger::log( "Stripe PaymentIntent $intent->id requires authentication for order $order_id" );
		}
	...
	public function save_intent_to_order( $order, $intent ) {
		$order->update_meta_data( '_Nbk_Cg_intent_id', $intent->id );
	...
	public function get_intent_from_order( $order ) {
		$intent_id = $order->get_meta( '_Nbk_Cg_intent_id' );
		...
		$intent_id = $order->get_meta( '_Nbk_Cg_setup_intent' );
	...
	private function get_intent( $intent_type, $intent_id ) {
		...
		$response = WC_Nbk_Cg_API::request( [], "$intent_type/$intent_id", 'GET' );

		if ( $response && isset( $response->{ 'error' } ) ) {
			$error_response_message = print_r( $response, true );
			WC_Nbk_Cg_Logger::log( "Failed to get Stripe intent $intent_type/$intent_id." );
			WC_Nbk_Cg_Logger::log( "Response: $error_response_message" );
			return false;
		}
	...
	public function lock_order_payment( $order, $intent = null ) {
		$order_id       = $order->get_id();
		$transient_name = 'wc_Nbk_Cg_processing_intent_' . $order_id;
	...
	public function unlock_order_payment( $order ) {
		$order_id = $order->get_id();
		delete_transient( 'wc_Nbk_Cg_processing_intent_' . $order_id );
	}
	...
	public function setup_intent( $order, $prepared_source ) {
		$order_id     = $order->get_id();
		$setup_intent = WC_Nbk_Cg_API::request(
			[
		...
		if ( is_wp_error( $setup_intent ) ) {
			WC_Nbk_Cg_Logger::log( "Unable to create SetupIntent for Order #$order_id: " . print_r( $setup_intent, true ) );
		} elseif ( 'requires_action' === $setup_intent->status ) {
			$order->update_meta_data( '_Nbk_Cg_setup_intent', $setup_intent->id );
			$order->save();
	...
	public function create_and_confirm_intent_for_off_session( $order, $prepared_source, $amount = null ) {
		...
		$request = [
			'amount'               => $amount ? WC_Nbk_Cg_Helper::get_nbk_cg_amount( $amount, $full_request['currency'] ) : $full_request['amount'],
		 ...
		 * @param object $source
		 */
		$request = apply_filters( 'wc_Nbk_Cg_generate_create_intent_request', $request, $order, $prepared_source );
		...
		$level3_data                = $this->get_level3_data_from_order( $order );
		$intent                     = WC_Nbk_Cg_API::request_with_level3_data(
			$request,
			'payment_intents',
			$level3_data,
			$order
		...
		$order_id       = $order->get_id();
		WC_Nbk_Cg_Logger::log( "Stripe PaymentIntent $intent_id initiated for order $order_id" );
	 ...
	 *
	 * Fix renewal for existing subscriptions affected by https://github.com/woocommerce/woocommerce-gateway-nbk-cg/issues/1072.
	 *
	...
	public function ensure_subscription_has_customer_id( $order_id ) {
		...
		foreach ( $subscriptions_ids as $subscription_id => $subscription ) {
			if ( ! metadata_exists( 'post', $subscription_id, '_nbk_cg_customer_id' ) ) {
				$stripe_customer = new WC_Nbk_Cg_Customer( $subscription->get_user_id() );
				update_post_meta( $subscription_id, '_nbk_cg_customer_id', $stripe_customer->get_id() );
				update_post_meta( $order_id, '_nbk_cg_customer_id', $stripe_customer->get_id() );
			}
		}
	}
