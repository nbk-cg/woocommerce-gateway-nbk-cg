 ...
 * @since 4.0.0
 */
class WC_Nbk_Cg_Order_Handler extends WC_Nbk_Cg_Payment_Gateway {
	...
	public function process_redirect_payment( $order_id, $retry = true, $previous_error = false ) {
		try {
			...
			WC_Nbk_Cg_Logger::log( "Info: (Redirect) Begin processing payment for order $order_id for the amount of {$order->get_total()}" );

			/**
			 * First check if the source is chargeable at this time. If not,
			 * webhook will take care of it later.
			 */
			$source_info = WC_Nbk_Cg_API::retrieve( 'sources/' . $source );

			if ( ! empty( $source_info->error ) ) {
				throw new WC_Nbk_Cg_Exception( print_r( $source_info, true ), $source_info->error->message );
			}

			if ( 'failed' === $source_info->status || 'canceled' === $source_info->status ) {
				throw new WC_Nbk_Cg_Exception( print_r( $source_info, true ), __( 'Unable to process this payment, please try again or use alternative method.', 'woocommerce-gateway-nbk-cg' ) );
			}
			...
			if ( ! empty( $response->error ) ) {
				...
				throw new WC_Nbk_Cg_Exception( print_r( $response, true ), $message );
			}
			...
			do_action( 'WC_Gateway_Nbk_Cgprocess_redirect_payment', $response, $order );
			...
			do_action( 'WC_Gateway_Nbk_Cgprocess_redirect_payment_error', $e, $order );

			/* translators: error message */
			$order->update_status( 'failed', sprintf( __( 'Stripe payment failed: %s', 'woocommerce-gateway-nbk-cg' ), $e->getLocalizedMessage() ) );
			...
			if ( $charge && 'no' === $captured ) {
				...
				if ( $intent ) {
					...
					if ( ! empty( $intent->error ) ) {
						/* translators: error message */
						$order->add_order_note( sprintf( __( 'Unable to capture charge! %s', 'woocommerce-gateway-nbk-cg' ), $intent->error->message ) );
					} elseif ( 'requires_capture' === $intent->status ) {
						$level3_data = $this->get_level3_data_from_order( $order );
						$result      = WC_Nbk_Cg_API::request_with_level3_data(
							[
								'amount'   => WC_Nbk_Cg_Helper::get_nbk_cg_amount( $order_total ),
						...
						if ( ! empty( $result->error ) ) {
							/* translators: error message */
							$order->update_status( 'failed', sprintf( __( 'Unable to capture charge! %s', 'woocommerce-gateway-nbk-cg' ), $result->error->message ) );
						} else {
							$is_Nbk_Cg_captured = true;
					...
					$result = WC_Nbk_Cg_API::retrieve( 'charges/' . $charge );

					if ( ! empty( $result->error ) ) {
						/* translators: error message */
						$order->add_order_note( sprintf( __( 'Unable to capture charge! %s', 'woocommerce-gateway-nbk-cg' ), $result->error->message ) );
				...
				if ( $is_Nbk_Cg_captured ) {
					/* translators: transaction id */
					$order->add_order_note( sprintf( __( 'Stripe charge complete (Charge ID: %s)', 'woocommerce-gateway-nbk-cg' ), $result->id ) );
				...
				do_action( 'woocommerce_Nbk_Cg_process_manual_capture', $order, $result );
			}
		}
	}
	...
	public function cancel_payment( $order_id ) {
		...
		if ( 'stripe' === $order->get_payment_method() ) {
			...
			do_action( 'woocommerce_Nbk_Cg_process_manual_cancel', $order );
		}
	}
	...
	public function woocommerce_tracks_event_properties( $properties, $prefixed_event_name ) {
		...
		$properties['woocommerce_gateway_Nbk_Cg_version'] = WC_NBK_CG_VERSION;
