 ...
 * @since 4.5.4
 */
class WC_Nbk_Cg_Inbox_Notes {
	const SUCCESS_NOTE_NAME = 'nbk-cg-apple-pay-marketing-guide-holiday-2020';
	const FAILURE_NOTE_NAME = 'nbk-cg-apple-pay-domain-verification-needed';

	const POST_SETUP_SUCCESS_ACTION    = 'wc_Nbk_Cg_apple_pay_post_setup_success';
	const CAMPAIGN_2020_CLEANUP_ACTION = 'wc_Nbk_Cg_apple_pay_2020_cleanup';
	...
	public static function get_success_title() {
		if ( time() < self::get_campaign_2020_cutoff() ) {
			return __( 'Boost sales this holiday season with Apple Pay!', 'woocommerce-gateway-nbk-cg' );
		}

		return __( 'Boost sales with Apple Pay!', 'woocommerce-gateway-nbk-cg' );
	}
	...
	public static function notify_on_apple_pay_domain_verification( $verification_complete ) {
		$admin_notes_class = WC_Nbk_Cg_Woo_Compat_Utils::get_notes_class();
	...
	public static function create_marketing_note() {
		...
		try {
			$admin_note_class = WC_Nbk_Cg_Woo_Compat_Utils::get_note_class();
			$note             = new $admin_note_class();
			$note->set_title( self::get_success_title() );
			$note->set_content( __( 'Now that you accept Apple Pay® with Stripe, you can increase conversion rates by letting your customers know that Apple Pay is available. Here’s a marketing guide to help you get started.', 'woocommerce-gateway-nbk-cg' ) );
			$note->set_type( $admin_note_class::E_WC_ADMIN_NOTE_MARKETING );
			$note->set_name( self::SUCCESS_NOTE_NAME );
			$note->set_source( 'woocommerce-gateway-nbk-cg' );
			$note->add_action(
				'marketing-guide',
				__( 'See marketing guide', 'woocommerce-gateway-nbk-cg' ),
	...
	public static function create_failure_note() {
		try {
			$admin_note_class = WC_Nbk_Cg_Woo_Compat_Utils::get_note_class();
			$note             = new $admin_note_class();
			$note->set_title( __( 'Apple Pay domain verification needed', 'woocommerce-gateway-nbk-cg' ) );
			$note->set_content( __( 'The WooCommerce Stripe Gateway extension attempted to perform domain verification on behalf of your store, but was unable to do so. This must be resolved before Apple Pay can be offered to your customers.', 'woocommerce-gateway-nbk-cg' ) );
			$note->set_type( $admin_note_class::E_WC_ADMIN_NOTE_INFORMATIONAL );
			$note->set_name( self::FAILURE_NOTE_NAME );
			$note->set_source( 'woocommerce-gateway-nbk-cg' );
			$note->add_action(
				'learn-more',
				__( 'Learn more', 'woocommerce-gateway-nbk-cg' ),
