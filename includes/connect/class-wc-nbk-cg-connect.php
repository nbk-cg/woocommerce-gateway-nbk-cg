...
if ( ! class_exists( 'WC_Nbk_Cg_Connect' ) ) {
	/**
	 * Stripe Connect class.
	 */
	class WC_Nbk_Cg_Connect {

		const SETTINGS_OPTION = 'woocommerce_nbk_cg_settings';
         ...
         * Constructor.
         *
         * @param WC_Nbk_Cg_Connect_API $api stripe connect api.
         */
		public function __construct( WC_Nbk_Cg_Connect_API $api ) {
			$this->api = $api;
		...
		public function get_oauth_url( $return_url = '' ) {
			...
			if ( empty( $return_url ) ) {
				$return_url = admin_url( 'admin.php?page=wc-settings&tab=checkout&section=nbk_cg' );
			}

			if ( substr( $return_url, 0, 8 ) !== 'https://' ) {
				return new WP_Error( 'invalid_url_protocol', __( 'Your site must be served over HTTPS in order to connect your Stripe account automatically.', 'woocommerce-gateway-nbk-cg' ) );
			}

			$result = $this->api->get_Nbk_Cg_oauth_init( $return_url );
		...
		public function connect_oauth( $state, $code ) {
			...
			$response = $this->api->get_Nbk_Cg_oauth_keys( $code );

			if ( is_wp_error( $response ) ) {
				return $response;
			}

			return $this->save_Nbk_Cg_keys( $response );
		}
		...
		public function maybe_handle_redirect() {
			...
			if ( isset( $_GET['wcs_Nbk_Cg_code'], $_GET['wcs_Nbk_Cg_state'] ) ) {

				$response = $this->connect_oauth( wc_clean( wp_unslash( $_GET['wcs_Nbk_Cg_state'] ) ), wc_clean( wp_unslash( $_GET['wcs_Nbk_Cg_code'] ) ) );
				wp_safe_redirect( remove_query_arg( [ 'wcs_Nbk_Cg_state', 'wcs_Nbk_Cg_code' ] ) );
				exit;

				// redirect from credentials reset
			} elseif ( isset( $_GET['reset_Nbk_Cg_api_credentials'], $_GET['_wpnonce'] ) ) {

				if ( ! wp_verify_nonce( wc_clean( wp_unslash( $_GET['_wpnonce'] ) ), 'reset_Nbk_Cg_api_credentials' ) ) {
					die( __( 'You are not authorized to clear Stripe account keys.', 'woocommerce-gateway-nbk-cg' ) );
				}

				$this->clear_Nbk_Cg_keys();
				wp_safe_redirect(
					remove_query_arg(
						[
							'_wpnonce',
							'reset_Nbk_Cg_api_credentials',
						]
					)
				);
				exit;
		 ...
		 */
		private function save_Nbk_Cg_keys( $result ) {
			...
			$default_options                        = $this->get_default_Nbk_Cg_config();
		 ...
		 */
		private function clear_Nbk_Cg_keys() {
		 ...
		 */
		private function get_default_Nbk_Cg_config() {
