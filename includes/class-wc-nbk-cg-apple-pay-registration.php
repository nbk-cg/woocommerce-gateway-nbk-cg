...
class WC_Nbk_Cg_Apple_Pay_Registration {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_Nbk_Cg_updated', [ $this, 'verify_domain_if_configured' ] );
		add_action( 'add_option_woocommerce_Nbk_Cg_settings', [ $this, 'verify_domain_on_new_settings' ], 10, 2 );
		add_action( 'update_option_woocommerce_Nbk_Cg_settings', [ $this, 'verify_domain_on_updated_settings' ], 10, 2 );
	...
	private function verify_hosted_domain_association_file_is_up_to_date() {
		...
		$new_contents = @file_get_contents( WC_NBK_CG_PLUGIN_PATH . '/' . self::DOMAIN_ASSOCIATION_FILE_NAME ); // @codingStandardsIgnoreLine
	...
	private function copy_and_overwrite_domain_association_file() {
		...
		if ( ! file_exists( $well_known_dir ) ) {
			if ( ! @mkdir( $well_known_dir, 0755 ) ) { // @codingStandardsIgnoreLine
				return __( 'Unable to create domain association folder to domain root.', 'woocommerce-gateway-nbk-cg' );
			}
		}

		if ( ! @copy( WC_NBK_CG_PLUGIN_PATH . '/' . self::DOMAIN_ASSOCIATION_FILE_NAME, $fullpath ) ) { // @codingStandardsIgnoreLine
			return __( 'Unable to copy domain association file to domain root.', 'woocommerce-gateway-nbk-cg' );
		}
	}
	...
	public function update_domain_association_file() {
		...
		if ( isset( $error_message ) ) {
			...
			WC_Nbk_Cg_Logger::log(
				...
				sprintf( __( 'To enable Apple Pay, domain association file must be hosted at %s.', 'woocommerce-gateway-nbk-cg' ), $url )
			);
		} else {
			WC_Nbk_Cg_Logger::log( __( 'Domain association file updated.', 'woocommerce-gateway-nbk-cg' ) );
		}
	}
	...
	public function parse_domain_association_request( $wp ) {
		...
		$path = WC_NBK_CG_PLUGIN_PATH . '/' . self::DOMAIN_ASSOCIATION_FILE_NAME;
	...
	private function make_domain_registration_request( $secret_key ) {
		if ( empty( $secret_key ) ) {
			throw new Exception( __( 'Unable to verify domain - missing secret key.', 'woocommerce-gateway-nbk-cg' ) );
		}
		...
		if ( is_wp_error( $response ) ) {
			...
			throw new Exception( sprintf( __( 'Unable to verify domain - %s', 'woocommerce-gateway-nbk-cg' ), $response->get_error_message() ) );
		}
		...
		if ( 200 !== $response['response']['code'] ) {
			...
			/* translators: error message */
			throw new Exception( sprintf( __( 'Unable to verify domain - %s', 'woocommerce-gateway-nbk-cg' ), $parsed_response->error->message ) );
		}
	}
	...
	public function register_domain_with_apple( $secret_key ) {
		try {
			...
			update_option( 'woocommerce_nbk_cg_settings', $this->stripe_settings );

			WC_Nbk_Cg_Logger::log( 'Your domain has been verified with Apple Pay!' );
	...
	public function verify_domain_if_configured() {
		...
		WC_Nbk_Cg_Inbox_Notes::notify_on_apple_pay_domain_verification( $verification_complete );
	}
	...
	public function admin_notices() {
		...
		$verification_failed_without_error = __( 'Apple Pay domain verification failed.', 'woocommerce-gateway-nbk-cg' );
		$verification_failed_with_error    = __( 'Apple Pay domain verification failed with the following error:', 'woocommerce-gateway-nbk-cg' );
		$check_log_text                    = sprintf(
			/* translators: 1) HTML anchor open tag 2) HTML anchor closing tag */
			esc_html__( 'Please check the %1$slogs%2$s for more details on this issue. Logging must be enabled to see recorded logs.', 'woocommerce-gateway-nbk-cg' ),
			'<a href="' . admin_url( 'admin.php?page=wc-status&tab=logs' ) . '">',
			'</a>'
		);

		?>
		<div class="error nbk-cg-apple-pay-message">
			<?php if ( $empty_notice ) : ?>
...
new WC_Nbk_Cg_Apple_Pay_Registration();
