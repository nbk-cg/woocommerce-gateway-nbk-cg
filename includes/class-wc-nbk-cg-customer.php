...
/**
 * WC_Nbk_Cg_Customer class.
 *
 * Represents a Stripe Customer.
 */
class WC_Nbk_Cg_Customer {
	...
	protected function generate_customer_request( $args = [] ) {
		...
		if ( $user ) {
			...
			$description = sprintf( __( 'Name: %1$s %2$s, Username: %3$s', 'woocommerce-gateway-nbk-cg' ), $billing_first_name, $billing_last_name, $user->user_login );
			...
			$description = sprintf( __( 'Name: %1$s %2$s, Guest', 'woocommerce-gateway-nbk-cg' ), $billing_first_name, $billing_last_name );
		...
		$metadata                      = [];
		$defaults['metadata']          = apply_filters( 'wc_Nbk_Cg_customer_metadata', $metadata, $user );
	...
	public function create_customer( $args = [] ) {
		...
		$response = WC_Nbk_Cg_API::request( apply_filters( 'wc_Nbk_Cg_create_customer_args', $args ), 'customers' );
		...
		do_action( 'woocommerce_Nbk_Cg_add_customer', $args, $response );
	...
	public function update_customer( $args = [], $is_retry = false ) {
		if ( empty( $this->get_id() ) ) {
			throw new WC_Nbk_Cg_Exception( 'id_required_to_update_user', __( 'Attempting to update a Stripe customer without a customer ID.', 'woocommerce-gateway-nbk-cg' ) );
		}

		$args     = $this->generate_customer_request( $args );
		$args     = apply_filters( 'wc_Nbk_Cg_update_customer_args', $args );
		$response = WC_Nbk_Cg_API::request( $args, 'customers/' . $this->get_id() );
		...
		do_action( 'woocommerce_Nbk_Cg_update_customer', $args, $response );
	...
	public function add_source( $source_id ) {
		$response = WC_Nbk_Cg_API::retrieve( 'sources/' . $source_id );
				...
				switch ( $response->type ) {
					...
					default:
						if ( 'source' === $response->object && 'card' === $response->type ) {
							...
							$wc_token->set_gateway_id( 'nbk-cg' );
		...
		do_action( 'woocommerce_Nbk_Cg_add_source', $this->get_id(), $wc_token, $response, $source_id );
	...
	public function attach_source( $source_id ) {
		...
		$response = WC_Nbk_Cg_API::request(
			[
				'source' => $source_id,
			],
		...
		if ( ! empty( $response->error ) ) {
			...
			if ( $this->is_no_such_customer_error( $response->error ) ) {
				...
				return WC_Nbk_Cg_API::request( [], 'sources/' . $source_id, 'GET' );
			} else {
				return $response;
			}
		} elseif ( empty( $response->id ) ) {
			return new WP_Error( 'error', __( 'Unable to add payment source.', 'woocommerce-gateway-nbk-cg' ) );
		} else {
			return $response;
		}
	}
	...
	public function delete_source( $source_id ) {
		...
		$response = WC_Nbk_Cg_API::request( [], 'customers/' . $this->get_id() . '/sources/' . sanitize_text_field( $source_id ), 'DELETE' );

		$this->clear_cache();

		if ( empty( $response->error ) ) {
			do_action( 'wc_Nbk_Cg_delete_source', $this->get_id(), $response );
	...
	public function set_default_source( $source_id ) {
		...
		if ( empty( $response->error ) ) {
			do_action( 'wc_Nbk_Cg_set_default_source', $this->get_id(), $response );
	...
	public function get_id_from_meta( $user_id ) {
		return get_user_option( '_nbk_cg_customer_id', $user_id );
	}
	...
	public function update_id_in_meta( $id ) {
		update_user_option( $this->get_user_id(), '_nbk_cg_customer_id', $id, false );
	}
	...
	public function delete_id_from_meta() {
		delete_user_option( $this->get_user_id(), '_nbk_cg_customer_id', false );
	}
