 ...
 */
class WC_Nbk_Cg_Email_Failed_Renewal_Authentication extends WC_Nbk_Cg_Email_Failed_Authentication {
	/**
	 * Constructor.
	 *
	...
	public function __construct( $email_classes = [] ) {
		...
		$this->title          = __( 'Failed Subscription Renewal SCA Authentication', 'woocommerce-gateway-nbk-cg' );
		$this->description    = __( 'Sent to a customer when a renewal fails because the transaction requires an SCA verification. The email contains renewal order information and payment links.', 'woocommerce-gateway-nbk-cg' );
		...
		add_action( 'WC_Gateway_Nbk_Cgprocess_payment_authentication_required', [ $this, 'trigger' ] );
	...
	public function get_default_subject() {
		return __( 'Payment authorization needed for renewal of {site_title} order {order_number}', 'woocommerce-gateway-nbk-cg' );
	}
	...
	public function get_default_heading() {
		return __( 'Payment authorization needed for renewal of order {order_number}', 'woocommerce-gateway-nbk-cg' );
	}
	...
	public function set_store_owner_custom_email( $rule_array, $retry_number, $order_id ) {
		if (
			...
			$rule_array['email_template_admin'] = 'WC_Nbk_Cg_Email_Failed_Authentication_Retry';
		}
