 ...
 */
class WC_Nbk_Cg_Subs_Compat extends WC_Gateway_Nbk_Cg {
	/**
	 * Constructor
	 */
	public function __construct() {
		...
		if ( class_exists( 'WC_Subscriptions_Order' ) ) {
			...
			add_action( 'wc_Nbk_Cg_cards_payment_fields', [ $this, 'display_update_subs_payment_checkout' ] );
			add_action( 'wc_Nbk_Cg_add_payment_method_' . $this->id . '_success', [ $this, 'handle_add_payment_method_success' ], 10, 2 );
			...
			add_filter( 'wc_Nbk_Cg_display_save_payment_method_checkbox', [ $this, 'maybe_hide_save_checkbox' ] );
	...
	public function display_update_subs_payment_checkout() {
		$subs_statuses = apply_filters( 'wc_Nbk_Cg_update_subs_payment_method_card_statuses', [ 'active' ] );
		if (
			apply_filters( 'wc_Nbk_Cg_display_update_subs_payment_method_card_checkbox', true ) &&
			wcs_user_has_subscription( get_current_user_id(), '', $subs_statuses ) &&
			is_add_payment_method_page()
		) {
			$label = esc_html( apply_filters( 'wc_Nbk_Cg_save_to_subs_text', __( 'Update the Payment Method used for all of my active subscriptions.', 'woocommerce-gateway-nbk-cg' ) ) );
			...
			woocommerce_form_field(
				...
				[
					'type'    => 'checkbox',
					'label'   => $label,
					'default' => apply_filters( 'wc_Nbk_Cg_save_to_subs_checked', false ),
				]
			);
		}
	}
				...
				foreach ( $all_subs as $sub ) {
					if ( $sub->has_status( $subs_statuses ) ) {
						WC_Subscriptions_Change_Payment_Gateway::update_payment_method(
							...
							[
								'post_meta' => [
									'_nbk_cg_source_id'   => [ 'value' => $source_id ],
									'_nbk_cg_customer_id' => [ 'value' => $stripe_customer->get_id() ],
								],
							]
						);
					}
	...
	public function differentiate_change_payment_method_form() {
		echo '<input type="hidden" id="wc-nbk-cg-change-payment-method" />';
	}
	...
	public function change_subs_payment_method( $order_id ) {
		try {
			...
			do_action( 'wc_Nbk_Cg_change_subs_payment_method_success', $prepared_source->source, $prepared_source );
	...
	/**
	 * Overloads WC_Nbk_Cg_Payment_Gateway::generate_create_intent_request() in order to
	...
	public function process_subscription_payment( $amount, $renewal_order, $retry = true, $previous_error = false ) {
		try {
			if ( $amount * 100 < WC_Nbk_Cg_Helper::get_minimum_amount() ) {
				/* translators: minimum amount */
				$message = sprintf( __( 'Sorry, the minimum allowed order total is %1$s to use this payment method.', 'woocommerce-gateway-nbk-cg' ), wc_price( WC_Nbk_Cg_Helper::get_minimum_amount() / 100 ) );
				throw new WC_Nbk_Cg_Exception(
			...
			if ( isset( $_REQUEST['process_early_renewal'] ) ) { // wpcs: csrf ok.
				...
				if ( 'success' === $response['result'] && isset( $response['payment_intent_secret'] ) ) {
					$verification_url = add_query_arg(
						...
						WC_AJAX::get_endpoint( 'wc_Nbk_Cg_verify_intent' )
					);
			...
			if ( ! $prepared_source->customer ) {
				throw new WC_Nbk_Cg_Exception(
					...
					__( 'Customer not found', 'woocommerce-gateway-nbk-cg' )
				);
			}

			WC_Nbk_Cg_Logger::log( "Info: Begin processing subscription payment for order {$order_id} for the amount of {$amount}" );
			...
			if ( is_object( $source_object ) && empty( $source_object->error ) && $this->need_update_idempotency_key( $source_object, $previous_error ) ) {
				add_filter( 'wc_Nbk_Cg_idempotency_key', [ $this, 'change_idempotency_key' ], 10, 2 );
			}

			if ( ( $this->is_no_such_source_error( $previous_error ) || $this->is_no_linked_source_error( $previous_error ) ) && apply_filters( 'wc_Nbk_Cg_use_default_customer_source', true ) ) {
			...
			if ( $is_authentication_required ) {
				do_action( 'WC_Gateway_Nbk_Cgprocess_payment_authentication_required', $renewal_order, $response );

				$error_message = __( 'This transaction requires authentication.', 'woocommerce-gateway-nbk-cg' );
				...
				$renewal_order->update_status( 'failed', sprintf( __( 'Stripe charge awaiting authentication by user: %s.', 'woocommerce-gateway-nbk-cg' ), $id ) );
				if ( is_callable( [ $renewal_order, 'save' ] ) ) {
					$renewal_order->save();
				}
			} else {
				// The charge was successfully captured
				do_action( 'WC_Gateway_Nbk_Cgprocess_payment', $response, $renewal_order );
			...
			do_action( 'WC_Gateway_Nbk_Cgprocess_payment_error', $e, $renewal_order );
	...
	public function save_source_to_order( $order, $source ) {
		...
		foreach ( $subscriptions as $subscription ) {
			...
			update_post_meta( $subscription_id, '_nbk_cg_customer_id', $source->customer );
			update_post_meta( $subscription_id, '_nbk_cg_source_id', $source->source );
		}
	}
	...
	public function delete_resubscribe_meta( $resubscribe_order ) {
		delete_post_meta( $resubscribe_order->get_id(), '_nbk_cg_customer_id' );
		delete_post_meta( $resubscribe_order->get_id(), '_nbk_cg_source_id' );
		// For BW compat will remove in future
		delete_post_meta( $resubscribe_order->get_id(), '_Nbk_Cg_card_id' );
		// delete payment intent ID
		delete_post_meta( $resubscribe_order->get_id(), '_Nbk_Cg_intent_id' );
	...
	public function delete_renewal_meta( $renewal_order ) {
		WC_Nbk_Cg_Helper::delete_Nbk_Cg_fee( $renewal_order );
		WC_Nbk_Cg_Helper::delete_Nbk_Cg_net( $renewal_order );

		// delete payment intent ID
		delete_post_meta( $renewal_order->get_id(), '_Nbk_Cg_intent_id' );
	...
	public function update_failing_payment_method( $subscription, $renewal_order ) {
		update_post_meta( $subscription->get_id(), '_nbk_cg_customer_id', $renewal_order->get_meta( '_nbk_cg_customer_id', true ) );
		update_post_meta( $subscription->get_id(), '_nbk_cg_source_id', $renewal_order->get_meta( '_nbk_cg_source_id', true ) );
	}
	...
	public function add_subscription_payment_meta( $payment_meta, $subscription ) {
		$subscription_id = $subscription->get_id();
		$source_id       = get_post_meta( $subscription_id, '_nbk_cg_source_id', true );

		// For BW compat will remove in future.
		if ( empty( $source_id ) ) {
			$source_id = get_post_meta( $subscription_id, '_Nbk_Cg_card_id', true );

			// Take this opportunity to update the key name.
			update_post_meta( $subscription_id, '_nbk_cg_source_id', $source_id );
			delete_post_meta( $subscription_id, '_Nbk_Cg_card_id', $source_id );
		}

		$payment_meta[ $this->id ] = [
			'post_meta' => [
				'_nbk_cg_customer_id' => [
					'value' => get_post_meta( $subscription_id, '_nbk_cg_customer_id', true ),
	...
	public function validate_subscription_payment_meta( $payment_method_id, $payment_meta ) {
		if ( $this->id === $payment_method_id ) {
			...
			if ( ! isset( $payment_meta['post_meta']['_nbk_cg_customer_id']['value'] ) || empty( $payment_meta['post_meta']['_nbk_cg_customer_id']['value'] ) ) {
				...
				if ( ! isset( $_POST['wc_order_action'] ) || 'wcs_process_renewal' !== $_POST['wc_order_action'] ) {
					throw new Exception( __( 'A "Stripe Customer ID" value is required.', 'woocommerce-gateway-nbk-cg' ) );
				}
			} elseif ( 0 !== strpos( $payment_meta['post_meta']['_nbk_cg_customer_id']['value'], 'cus_' ) ) {
				throw new Exception( __( 'Invalid customer ID. A valid "Stripe Customer ID" must begin with "cus_".', 'woocommerce-gateway-nbk-cg' ) );
			}

			if (
				! empty( $payment_meta['post_meta']['_nbk_cg_source_id']['value'] ) && (
					0 !== strpos( $payment_meta['post_meta']['_nbk_cg_source_id']['value'], 'card_' )
					&& 0 !== strpos( $payment_meta['post_meta']['_nbk_cg_source_id']['value'], 'src_' )
					&& 0 !== strpos( $payment_meta['post_meta']['_nbk_cg_source_id']['value'], 'pm_' )
				)
			) {
				throw new Exception( __( 'Invalid source ID. A valid source "Stripe Source ID" must begin with "src_", "pm_", or "card_".', 'woocommerce-gateway-nbk-cg' ) );
			}
		}
	}
	...
	public function maybe_render_subscription_payment_method( $payment_method_to_display, $subscription ) {
		...
		$stripe_source_id = get_post_meta( $subscription->get_id(), '_nbk_cg_source_id', true );

		// For BW compat will remove in future.
		if ( empty( $stripe_source_id ) ) {
			$stripe_source_id = get_post_meta( $subscription->get_id(), '_Nbk_Cg_card_id', true );

			// Take this opportunity to update the key name.
			update_post_meta( $subscription->get_id(), '_nbk_cg_source_id', $stripe_source_id );
		}

		$stripe_customer    = new WC_Nbk_Cg_Customer();
		$stripe_customer_id = get_post_meta( $subscription->get_id(), '_nbk_cg_customer_id', true );

		// If we couldn't find a Stripe customer linked to the subscription, fallback to the user meta data.
		if ( ! $stripe_customer_id || ! is_string( $stripe_customer_id ) ) {
			$user_id            = $customer_user;
			$stripe_customer_id = get_user_option( '_nbk_cg_customer_id', $user_id );
			$stripe_source_id   = get_user_option( '_nbk_cg_source_id', $user_id );

			// For BW compat will remove in future.
			if ( empty( $stripe_source_id ) ) {
				$stripe_source_id = get_user_option( '_Nbk_Cg_card_id', $user_id );

				// Take this opportunity to update the key name.
				update_user_option( $user_id, '_nbk_cg_source_id', $stripe_source_id, false );
			}
		}

		// If we couldn't find a Stripe customer linked to the account, fallback to the order meta data.
		if ( ( ! $stripe_customer_id || ! is_string( $stripe_customer_id ) ) && false !== $subscription->get_parent() ) {
			$stripe_customer_id = get_post_meta( $subscription->get_parent_id(), '_nbk_cg_customer_id', true );
			$stripe_source_id   = get_post_meta( $subscription->get_parent_id(), '_nbk_cg_source_id', true );

			// For BW compat will remove in future.
			if ( empty( $stripe_source_id ) ) {
				$stripe_source_id = get_post_meta( $subscription->get_parent_id(), '_Nbk_Cg_card_id', true );

				// Take this opportunity to update the key name.
				update_post_meta( $subscription->get_parent_id(), '_nbk_cg_source_id', $stripe_source_id );
			}
		}
		...
		$payment_method_to_display = __( 'N/A', 'woocommerce-gateway-nbk-cg' );
			...
			foreach ( $sources as $source ) {
				...
				if ( $source->id === $stripe_source_id ) {
					if ( $card ) {
						...
						$payment_method_to_display = sprintf( __( 'Via %1$s card ending in %2$s', 'woocommerce-gateway-nbk-cg' ), ( isset( $card->brand ) ? $card->brand : __( 'N/A', 'woocommerce-gateway-nbk-cg' ) ), $card->last4 );
					}
	...
	public function remove_order_pay_var() {
		global $wp;
		if ( isset( $_GET['wc-nbk-cg-confirmation'] ) ) {
	...
	public function has_authentication_already_failed( $renewal_order ) {
		 ...
		 */
		do_action( 'WC_Gateway_Nbk_Cgprocess_payment_authentication_required', $renewal_order );
		...
		$renewal_order->update_status( 'failed', sprintf( __( 'Stripe charge awaiting authentication by user: %s.', 'woocommerce-gateway-nbk-cg' ), $charge_id ) );
	...
	protected function handle_intent_verification_success( $order, $intent ) {
		...
		if ( isset( $_GET['early_renewal'] ) ) { // wpcs: csrf ok.
			...
			wc_add_notice( __( 'Your early renewal order was successful.', 'woocommerce-gateway-nbk-cg' ), 'success' );
		}
	}
	...
	protected function handle_intent_verification_failure( $order, $intent ) {
		if ( isset( $_GET['early_renewal'] ) ) {
			$order->delete( true );
			wc_add_notice( __( 'Payment authorization for the renewal order was unsuccessful, please try again.', 'woocommerce-gateway-nbk-cg' ), 'error' );
