...
class WC_Nbk_Cg_Intent_Controller {
	...
	public function __construct() {
		add_action( 'wc_ajax_wc_Nbk_Cg_verify_intent', [ $this, 'verify_intent' ] );
		add_action( 'wc_ajax_wc_Nbk_Cg_create_setup_intent', [ $this, 'create_setup_intent' ] );
	}
	 ...
	 *
	 * @since 4.2.0
	 * @return WC_Gateway_Nbk_Cg
	 */
	protected function get_gateway() {
		if ( ! isset( $this->gateway ) ) {
			if ( class_exists( 'WC_Subscriptions_Order' ) && function_exists( 'wcs_create_renewal_order' ) ) {
				$class_name = 'WC_Nbk_Cg_Subs_Compat';
			} else {
				$class_name = 'WC_Gateway_Nbk_Cg';
			}
	 ...
	 *
	 * @since 4.2.0
	 * @throws WC_Nbk_Cg_Exception An exception if there is no order ID or the order does not exist.
	 * @return WC_Order
	 */
	protected function get_order_from_request() {
		if ( ! isset( $_GET['nonce'] ) || ! wp_verify_nonce( sanitize_key( $_GET['nonce'] ), 'wc_Nbk_Cg_confirm_pi' ) ) {
			throw new WC_Nbk_Cg_Exception( 'missing-nonce', __( 'CSRF verification failed.', 'woocommerce-gateway-nbk-cg' ) );
		}
		...
		if ( ! $order ) {
			throw new WC_Nbk_Cg_Exception( 'missing-order', __( 'Missing order ID for payment confirmation', 'woocommerce-gateway-nbk-cg' ) );
		}
	...
	public function verify_intent() {
		...
		try {
			...
			$message = sprintf( __( 'Payment verification error: %s', 'woocommerce-gateway-nbk-cg' ), $e->getLocalizedMessage() );
		...
		try {
			...
			if ( isset( $_GET['save_payment_method'] ) && ! empty( $_GET['save_payment_method'] ) ) {
				...
				if ( isset( $intent->last_payment_error ) ) {
					...
					$customer = new WC_Nbk_Cg_Customer( wp_get_current_user()->ID );
					...
					if ( isset( $metadata->save_payment_method ) && 'true' === $metadata->save_payment_method ) {
						$source_object = WC_Nbk_Cg_API::retrieve( 'sources/' . $intent->source );
	 ...
	 *
	 * @since 4.2.0
	 * @param WC_Nbk_Cg_Exception $e           The exception that was thrown.
	...
	public function create_setup_intent() {
		...
		try {
			...
			// 1. Verify.
			if (
				! wp_verify_nonce( sanitize_key( $_POST['nonce'] ), 'wc_Nbk_Cg_create_si' )
				|| ! preg_match( '/^src_.*$/', $source_id )
			) {
				throw new Exception( __( 'Unable to verify your request. Please reload the page and try again.', 'woocommerce-gateway-nbk-cg' ) );
			}
			...
			if ( ! empty( $setup_intent->error ) ) {
				...
				WC_Nbk_Cg_Logger::log( 'Failed create Setup Intent while saving a card.' );
				WC_Nbk_Cg_Logger::log( "Response: $error_response_message" );
				throw new Exception( __( 'Your card could not be set up for future usage.', 'woocommerce-gateway-nbk-cg' ) );
			}
			...
			if ( 'requires_action' === $setup_intent->status ) {
				...
				$response = [
					...
					'error'  => [
						...
						'message' => __( 'Failed to save payment method.', 'woocommerce-gateway-nbk-cg' ),
					],
				];
			} else {
...
new WC_Nbk_Cg_Intent_Controller();
