<?php
/**
 * Nbk-cg Payment Request API
 * Adds support for Apple Pay and Chrome Payment Request API buttons.
 * Utilizes the Nbk-cg Payment Request Button to support checkout from the product detail and cart pages.
 *
 * @package WooCommerce_Nbk_Cg/Classes/Payment_Request
 * @since   4.0.0
 */
...
/**
 * wc_nbk_cg_payment_request class.
 */
class wc_nbk_cg_payment_request {
	/**
	 * Enabled.
	 *
	 * @var
	...
	public function __construct() {
		self::$_this           = $this;
		$this->stripe_settings = get_option( 'woocommerce_nbk_cg_settings', [] );
		$this->testmode        = ( ! empty( $this->stripe_settings['testmode'] ) && 'yes' === $this->stripe_settings['testmode'] ) ? true : false;
		$this->publishable_key = ! empty( $this->stripe_settings['publishable_key'] ) ? $this->stripe_settings['publishable_key'] : '';
		$this->secret_key      = ! empty( $this->stripe_settings['secret_key'] ) ? $this->stripe_settings['secret_key'] : '';
		$this->total_label     = ! empty( $this->stripe_settings['statement_descriptor'] ) ? WC_Nbk_Cg_Helper::clean_statement_descriptor( $this->stripe_settings['statement_descriptor'] ) : '';



        $this->account_id = ! empty( $this->stripe_settings['account_id'] ) ? $this->stripe_settings['account_id'] : '';
        $this->test_client_id      = ! empty( $this->stripe_settings['test_client_id'] ) ? $this->stripe_settings['test_client_id'] : '';
        $this->test_client_secret     = ! empty( $this->stripe_settings['test_client_secret'] ) ? $this->stripe_settings['test_client_secret'] : '';
        $this->client_id = ! empty( $this->stripe_settings['client_id'] ) ? $this->stripe_settings['client_id'] : '';
        $this->client_secret      = ! empty( $this->stripe_settings['client_secret'] ) ? $this->stripe_settings['client_secret'] : '';
		...
		$this->total_label = str_replace( "'", '', $this->total_label ) . apply_filters( 'wc_Nbk_Cg_payment_request_total_label_suffix', ' (via WooCommerce)' );
	...
	public function are_keys_set() {
		...
		// in includes/abstracts/abstract-wc-nbk-cg-payment-gateway.php
		/*if ( $this->testmode ) {
	...
	public function handle_payment_request_redirect() {
		if (
			! empty( $_GET['wc_Nbk_Cg_payment_request_redirect_url'] )
			&& ! empty( $_GET['_wpnonce'] )
			&& wp_verify_nonce( $_GET['_wpnonce'], 'wc-nbk-cg-set-redirect-url' ) // @codingStandardsIgnoreLine
		) {
			$url = rawurldecode( esc_url_raw( wp_unslash( $_GET['wc_Nbk_Cg_payment_request_redirect_url'] ) ) );
			...
			wc_setcookie( 'wc_Nbk_Cg_payment_request_redirect_url', $url, time() + MINUTE_IN_SECONDS * 10 );
	...
	public function init() {
		...
		add_action( 'wc_ajax_wc_Nbk_Cg_get_cart_details', [ $this, 'ajax_get_cart_details' ] );
		add_action( 'wc_ajax_wc_Nbk_Cg_get_shipping_options', [ $this, 'ajax_get_shipping_options' ] );
		add_action( 'wc_ajax_wc_Nbk_Cg_update_shipping_method', [ $this, 'ajax_update_shipping_method' ] );
		add_action( 'wc_ajax_wc_Nbk_Cg_create_order', [ $this, 'ajax_create_order' ] );
		add_action( 'wc_ajax_wc_Nbk_Cg_add_to_cart', [ $this, 'ajax_add_to_cart' ] );
		add_action( 'wc_ajax_wc_Nbk_Cg_log_errors', [ $this, 'ajax_log_errors' ] );
	...
	public function custom_button_selector() {
		return $this->is_custom_button() ? '#wc-nbk-cg-custom-button' : '';
	}
	...
	public function get_product_data() {
		...
		$data                    = [];
		$data['total']           = [
			'label'   => apply_filters( 'wc_Nbk_Cg_payment_request_total_label', $this->total_label ),
			'amount'  => WC_Nbk_Cg_Helper::get_nbk_cg_amount( $product->get_price() ),
			'pending' => true,
		];
		$data['requestShipping'] = ( wc_shipping_enabled() && $product->needs_shipping() );

		return apply_filters( 'wc_Nbk_Cg_payment_request_product_data', $data, $product );
	}
	...
	public function supported_product_types() {
		return apply_filters(
			'wc_Nbk_Cg_payment_request_supported_types',
			[
				'simple',
				'variable',
				'variation',
	...
	public function has_subscription_product() {
		...
		} elseif ( WC_Nbk_Cg_Helper::has_cart_or_checkout_on_current_page() ) {
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				$_product = apply_filters( 'woocommerce_cart_item_product', $cart_item['data'], $cart_item, $cart_item_key );
	...
	public function get_login_redirect_url( $redirect ) {
		$url = esc_url_raw( wp_unslash( isset( $_COOKIE['wc_Nbk_Cg_payment_request_redirect_url'] ) ? $_COOKIE['wc_Nbk_Cg_payment_request_redirect_url'] : '' ) );

		if ( empty( $url ) ) {
			return $redirect;
		}
		wc_setcookie( 'wc_Nbk_Cg_payment_request_redirect_url', null );
	...
	public function javascript_params() {
		...
		return [
			...
			'stripe'             => [
				...
				'allow_prepaid_card' => apply_filters( 'wc_Nbk_Cg_allow_prepaid_card', true ) ? 'yes' : 'no',
			],
			'nonce'              => [
				'payment'         => wp_create_nonce( 'wc-nbk-cg-payment-request' ),
				'shipping'        => wp_create_nonce( 'wc-nbk-cg-payment-request-shipping' ),
				'update_shipping' => wp_create_nonce( 'wc-nbk-cg-update-shipping-method' ),
				'checkout'        => wp_create_nonce( 'woocommerce-process_checkout' ),
				'add_to_cart'     => wp_create_nonce( 'wc-nbk-cg-add-to-cart' ),
				'log_errors'      => wp_create_nonce( 'wc-nbk-cg-log-errors' ),
			],
			'i18n'               => [
				'no_prepaid_card'  => __( 'Sorry, we\'re not accepting prepaid cards at this time.', 'woocommerce-gateway-nbk-cg' ),
				/* translators: Do not translate the [option] placeholder */
				'unknown_shipping' => __( 'Unknown shipping option "[option]".', 'woocommerce-gateway-nbk-cg' ),
			],
			'checkout'           => [
			...
			'button'             => [
				...
				'locale'       => apply_filters( 'wc_Nbk_Cg_payment_request_button_locale', substr( get_locale(), 0, 2 ) ), // Default format is en_US.
	...
	public function scripts() {
		...
		wp_register_script( 'nbk-cg', 'https://js.stripe.com/v3', '', '3.0', true );
		//wp_register_script( 'wc_nbk_cg_payment_request', plugins_url( 'assets/js/nbk-cg-payment-request' . $suffix . '.js', WC_NBK_CG_MAIN_FILE ), [ 'jquery', 'stripe' ], WC_NBK_CG_VERSION, true );

		wp_localize_script(
			'wc_nbk_cg_payment_request',
		...
		wp_enqueue_script( 'wc_nbk_cg_payment_request' );

		$gateways = WC()->payment_gateways->get_available_payment_gateways();

		if ( isset( $gateways['nbk_cg'] ) ) {
			$gateways['nbk_cg']->payment_scripts();
		}
	}
	...
	public function display_payment_request_button_html() {
		...
		?>
		<div id="wc-nbk-cg-payment-request-wrapper" style="clear:both;padding-top:1.5em;display:none;">
			<div id="wc-nbk-cg-payment-request-button">
				<?php
				if ( $this->is_custom_button() ) {
					...
					echo "<button id=\"wc-nbk-cg-custom-button\" class=\"$class_name\" style=\"$style\"> $label </button>";
				}
				?>
	...
	public function display_payment_request_button_separator_html() {
		...
		if ( is_checkout() && ! apply_filters( 'wc_nbk_cg_show_payment_request_on_checkout', false, $post ) ) {
			return;
		}
		?>
		<p id="wc-nbk-cg-payment-request-button-separator" style="margin-top:1.5em;text-align:center;display:none;">&mdash; <?php esc_html_e( 'OR', 'woocommerce-gateway-nbk-cg' ); ?> &mdash;</p>
		<?php
	}
	...
	public function should_show_payment_request_button() {
		...
		/*if ( ! $this->testmode && ! is_ssl() ) {
			WC_Nbk_Cg_Logger::log( 'Stripe Payment Request live mode requires SSL.' );
			return false;
		}*/
		...
		if ( is_cart() && ! apply_filters( 'wc_Nbk_Cg_show_payment_request_on_cart', true ) ) {
			return false;
		}
		...
		if (
			$this->is_product()
			&& apply_filters( 'wc_Nbk_Cg_hide_payment_request_on_product_page', false, $post )
		) {
			return false;
		}
	...
	public function ajax_log_errors() {
		check_ajax_referer( 'wc-nbk-cg-log-errors', 'security' );

		$errors = isset( $_POST['errors'] ) ? wc_clean( wp_unslash( $_POST['errors'] ) ) : '';

		WC_Nbk_Cg_Logger::log( $errors );
	...
	public function ajax_get_cart_details() {
		check_ajax_referer( 'wc-nbk-cg-payment-request', 'security' );
	...
	public function ajax_get_shipping_options() {
		check_ajax_referer( 'wc-nbk-cg-payment-request-shipping', 'security' );
	...
	public function get_shipping_options( $shipping_address, $itemized_display_items = false ) {
		...
		try {
			...
			$this->calculate_shipping( apply_filters( 'wc_Nbk_Cg_payment_request_shipping_posted_values', $shipping_address ) );
			...
			if ( ! empty( $packages ) && WC()->customer->has_calculated_shipping() ) {
				foreach ( $packages as $package_key => $package ) {
					if ( empty( $package['rates'] ) ) {
						throw new Exception( __( 'Unable to find shipping method for address.', 'woocommerce-gateway-nbk-cg' ) );
					}
					...
					foreach ( $package['rates'] as $key => $rate ) {
						if ( in_array( $rate->id, $shipping_rate_ids, true ) ) {
							...
							// option IDs.
							throw new Exception( __( 'Unable to provide shipping options for Payment Requests.', 'woocommerce-gateway-nbk-cg' ) );
						}
						...
						$data['shipping_options'][] = [
							...
							'label'  => $rate->label,
							'detail' => '',
							'amount' => WC_Nbk_Cg_Helper::get_nbk_cg_amount( $rate->cost ),
						];
					}
				}
			} else {
	...
	public function ajax_update_shipping_method() {
		check_ajax_referer( 'wc-nbk-cg-update-shipping-method', 'security' );
	...
	public function ajax_add_to_cart() {
		check_ajax_referer( 'wc-nbk-cg-add-to-cart', 'security' );
		...
		try {
			...
			if ( ! is_a( $product, 'WC_Product' ) ) {
				...
				throw new Exception( sprintf( __( 'Product with the ID (%d) cannot be found.', 'woocommerce-gateway-nbk-cg' ), $product_id ) );
			}
			...
			if ( ! $is_in_stock ) {
				throw new Exception( __( 'Sorry, this product is unavailable. Please choose a different combination.', 'woocommerce-gateway-nbk-cg' ) );
			}

			if ( ! $has_enough_stock ) {
				/* translators: 1: product name 2: quantity in stock */
				throw new Exception( sprintf( __( 'You cannot add that amount of "%1$s"; to the cart because there is not enough stock (%2$s remaining).', 'woocommerce-gateway-nbk-cg' ), $product->get_name(), $stock_qty_for_display ) );
			}
	...
	public function get_normalized_state_from_pr_states( $state, $country ) {
		...
		include_once WC_NBK_CG_PLUGIN_PATH . '/includes/constants/class-wc-nbk-cg-payment-request-button-states.php';
		$pr_states = WC_Nbk_Cg_Payment_Request_Button_States::STATES;
		...
		if ( ! $is_supported ) {
			wc_add_notice(
				sprintf(
					...
					__( 'The Payment Request button is not supported in %s because some required fields couldn\'t be verified. Please proceed to the checkout page and try again.', 'woocommerce-gateway-nbk-cg' ),
	...
	public function ajax_create_order() {
		if ( WC()->cart->is_empty() ) {
			wp_send_json_error( __( 'Empty cart', 'woocommerce-gateway-nbk-cg' ) );
		}
	...
	protected function build_shipping_methods( $shipping_methods ) {
		...
		foreach ( $shipping_methods as $method ) {
			$shipping[] = [
				...
				'label'  => $method['label'],
				'detail' => '',
				'amount' => WC_Nbk_Cg_Helper::get_nbk_cg_amount( $method['amount']['value'] ),
			];
		}
	...
	protected function build_response( $itemized_display_items = false, $has_shipping_address = false ) {
		...
		if ( ! apply_filters( 'wc_Nbk_Cg_payment_request_hide_itemization', true ) || $itemized_display_items ) {
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				$items[] = [
					...
					'amount' => WC_Nbk_Cg_Helper::get_nbk_cg_amount( $amount ),
				];
			}
		}
		...
		if ( wc_tax_enabled() ) {
			...
			$items[] = [
				'label'   => esc_html( __( 'Tax', 'woocommerce-gateway-nbk-cg' ) ),
				'amount'  => WC_Nbk_Cg_Helper::get_nbk_cg_amount( $tax ),
				'pending' => true,
			];
		}
		...
		if ( wc_shipping_enabled() && WC()->cart->needs_shipping() ) {
			...
			$items[] = [
				'label'   => esc_html( __( 'Shipping', 'woocommerce-gateway-nbk-cg' ) ),
				'amount'  => WC_Nbk_Cg_Helper::get_nbk_cg_amount( $shipping ),
				'pending' => true,
			];
		}
		...
		if ( WC()->cart->has_discount() ) {
			$items[] = [
				'label'  => esc_html( __( 'Discount', 'woocommerce-gateway-nbk-cg' ) ),
				'amount' => WC_Nbk_Cg_Helper::get_nbk_cg_amount( $discounts ),
			];
		}
		...
		foreach ( $cart_fees as $key => $fee ) {
			$items[] = [
				'label'  => $fee->name,
				'amount' => WC_Nbk_Cg_Helper::get_nbk_cg_amount( $fee->amount ),
			];
		}
		...
		$data['total'] = [
			...
			'amount'  => max( 0, apply_filters( 'woocommerce_Nbk_Cg_calculated_total', WC_Nbk_Cg_Helper::get_nbk_cg_amount( $order_total ), $order_total, WC()->cart ) ),
			'pending' => false,
		];
	...
	public function get_login_confirmation_settings() {
		...
		$message      = __( 'To complete your transaction with **the selected payment method**, you must log in or create an account with our site.', 'woocommerce-gateway-nbk-cg' );
		$redirect_url = add_query_arg(
			[
				'_wpnonce'                               => wp_create_nonce( 'wc-nbk-cg-set-redirect-url' ),
				'wc_Nbk_Cg_payment_request_redirect_url' => rawurlencode( home_url( add_query_arg( [] ) ) ), // Current URL to redirect to after login.
			],
			home_url()
		);
