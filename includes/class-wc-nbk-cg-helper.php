 ...
 * @since 4.0.0
 */
class WC_Nbk_Cg_Helper {
	const LEGACY_META_NAME_FEE      = 'Stripe Fee';
	const LEGACY_META_NAME_NET      = 'Net Revenue From Stripe';
	const META_NAME_FEE             = '_Nbk_Cg_fee';
	const META_NAME_NET             = '_Nbk_Cg_net';
	const META_NAME_Nbk_Cg_CURRENCY = '_Nbk_Cg_currency';
	 ...
	 */
	public static function get_Nbk_Cg_currency( $order = null ) {
		if ( is_null( $order ) ) {
			return false;
		}

		return $order->get_meta( self::META_NAME_Nbk_Cg_CURRENCY, true );
	}
	 ...
	 */
	public static function update_Nbk_Cg_currency( $order, $currency ) {
		if ( is_null( $order ) ) {
			return false;
		}

		$order->update_meta_data( self::META_NAME_Nbk_Cg_CURRENCY, $currency );
	}
	 ...
	 */
	public static function get_Nbk_Cg_fee( $order = null ) {
		if ( is_null( $order ) ) {
			return false;
		}
		...
		if ( empty( $amount ) ) {
			...
			if ( $amount ) {
				self::update_Nbk_Cg_fee( $order, $amount );
			}
		}
	 ...
	 * @param float  $amount
	 */
	public static function update_Nbk_Cg_fee( $order = null, $amount = 0.0 ) {
		if ( is_null( $order ) ) {
			return false;
		}
	 ...
	 * @param object $order
	 */
	public static function delete_Nbk_Cg_fee( $order = null ) {
		if ( is_null( $order ) ) {
			return false;
		}
	 ...
	 * @return string $amount
	 */
	public static function get_Nbk_Cg_net( $order = null ) {
		if ( is_null( $order ) ) {
			return false;
		}
		...
		if ( empty( $amount ) ) {
			...
			// If found update to new name.
			if ( $amount ) {
				self::update_Nbk_Cg_net( $order, $amount );
			}
		}
	 ...
	 * @param float  $amount
	 */
	public static function update_Nbk_Cg_net( $order = null, $amount = 0.0 ) {
		if ( is_null( $order ) ) {
			return false;
		}
	 ...
	 * @param object $order
	 */
	public static function delete_Nbk_Cg_net( $order = null ) {
		if ( is_null( $order ) ) {
			return false;
		}
	 ...
	 * @return float|int
	 */
	public static function get_nbk_cg_amount( $total, $currency = '' ) {
		if ( ! $currency ) {
	...
	public static function get_localized_messages() {
		return apply_filters(
			'wc_Nbk_Cg_localized_messages',
			[
				'invalid_number'           => __( 'The card number is not a valid credit card number.', 'woocommerce-gateway-nbk-cg' ),
				'invalid_expiry_month'     => __( 'The card\'s expiration month is invalid.', 'woocommerce-gateway-nbk-cg' ),
				'invalid_expiry_year'      => __( 'The card\'s expiration year is invalid.', 'woocommerce-gateway-nbk-cg' ),
				'invalid_cvc'              => __( 'The card\'s security code is invalid.', 'woocommerce-gateway-nbk-cg' ),
				'incorrect_number'         => __( 'The card number is incorrect.', 'woocommerce-gateway-nbk-cg' ),
				'incomplete_number'        => __( 'The card number is incomplete.', 'woocommerce-gateway-nbk-cg' ),
				'incomplete_cvc'           => __( 'The card\'s security code is incomplete.', 'woocommerce-gateway-nbk-cg' ),
				'incomplete_expiry'        => __( 'The card\'s expiration date is incomplete.', 'woocommerce-gateway-nbk-cg' ),
				'expired_card'             => __( 'The card has expired.', 'woocommerce-gateway-nbk-cg' ),
				'incorrect_cvc'            => __( 'The card\'s security code is incorrect.', 'woocommerce-gateway-nbk-cg' ),
				'incorrect_zip'            => __( 'The card\'s zip code failed validation.', 'woocommerce-gateway-nbk-cg' ),
				'invalid_expiry_year_past' => __( 'The card\'s expiration year is in the past', 'woocommerce-gateway-nbk-cg' ),
				'card_declined'            => __( 'The card was declined.', 'woocommerce-gateway-nbk-cg' ),
				'missing'                  => __( 'There is no card on a customer that is being charged.', 'woocommerce-gateway-nbk-cg' ),
				'processing_error'         => __( 'An error occurred while processing the card.', 'woocommerce-gateway-nbk-cg' ),
				'invalid_sofort_country'   => __( 'The billing country is not accepted by SOFORT. Please try another country.', 'woocommerce-gateway-nbk-cg' ),
				'email_invalid'            => __( 'Invalid email address, please correct and try again.', 'woocommerce-gateway-nbk-cg' ),
				'invalid_request_error'    => is_add_payment_method_page()
					? __( 'Unable to save this payment method, please try again or use alternative method.', 'woocommerce-gateway-nbk-cg' )
					: __( 'Unable to process this payment, please try again or use alternative method.', 'woocommerce-gateway-nbk-cg' ),
			]
		);
	}
	...
	public static function get_settings( $method = null, $setting = null ) {
		$all_settings = null === $method ? get_option( 'woocommerce_nbk_cg_settings', [] ) : get_option( 'woocommerce_Nbk_Cg_' . $method . '_settings', [] );
	...
	public static function get_webhook_url() {
		return add_query_arg( 'wc-api', 'wc_nbk_cg', trailingslashit( get_home_url() ) );
	}
	...
	public static function get_order_by_source_id( $source_id ) {
		...
		$order_id = $wpdb->get_var( $wpdb->prepare( "SELECT DISTINCT ID FROM $wpdb->posts as posts LEFT JOIN $wpdb->postmeta as meta ON posts.ID = meta.post_id WHERE meta.meta_value = %s AND meta.meta_key = %s", $source_id, '_nbk_cg_source_id' ) );
	...
	public static function get_order_by_intent_id( $intent_id ) {
		...
		$order_id = $wpdb->get_var( $wpdb->prepare( "SELECT DISTINCT ID FROM $wpdb->posts as posts LEFT JOIN $wpdb->postmeta as meta ON posts.ID = meta.post_id WHERE meta.meta_value = %s AND meta.meta_key = %s", $intent_id, '_Nbk_Cg_intent_id' ) );
	...
	public static function get_order_by_setup_intent_id( $intent_id ) {
		...
		$order_id = $wpdb->get_var( $wpdb->prepare( "SELECT DISTINCT ID FROM $wpdb->posts as posts LEFT JOIN $wpdb->postmeta as meta ON posts.ID = meta.post_id WHERE meta.meta_value = %s AND meta.meta_key = %s", $intent_id, '_Nbk_Cg_setup_intent' ) );
	 ...
	 */
	public static function convert_wc_locale_to_Nbk_Cg_locale( $wc_locale ) {
